...
bool contains(const std::vector<char*>& charArray, const char* target) {
  if (charArray.empty()) {
        return true;  // ÎπÑÏñ¥ ?ûà?úºÎ©? false Î∞òÌôò
    }
    // std::find_ifÎ•? ?Ç¨?ö©?ïò?ó¨ targetÍ≥? Í∞ôÏ?? Î¨∏Ïûê?ó¥?ùÑ Ï∞æÏäµ?ãà?ã§.
    auto it = std::find_if(charArray.begin(), charArray.end(), 
        [target](const char* str) {
            return strcmp(str, target) == 0;  // Î¨∏Ïûê?ó¥ ÎπÑÍµê
        });

    return it != charArray.end();  // Ï∞æÏúºÎ©? true, ?ïÑ?ãàÎ©? false
}

void clearCharArray() {
    for (char* str : charArray) {
        delete[] str; // Í∞? ?è¨?ù∏?Ñ∞Î°? ?ï†?ãπ?êú Î©îÎ™®Î¶? ?ï¥?†ú
    }
    charArray.clear(); // Î≤°ÌÑ∞ ÎπÑÏö∞Í∏? (?Ñ†?Éù ?Ç¨?ï≠)
}

bool contains(char* arr[], int length, const char* target) {
    for (int i = 0; i < length; ++i) {
        if (strcmp(arr[i], target) == 0) {
            return true;  // Ï∞æÏúºÎ©? true Î∞òÌôò
        }
    }
    return false;  // Ï∞æÏ?? Î™ªÌïòÎ©? false Î∞òÌôò
}
...
void parseEntitySpline(Dwg_Object* object, Napi::Array jsonArr, Napi::Env env, bool isCheckLayer) {
  ...
  for(BITCODE_BS i = 0; i < spline->num_fit_pts; i++){
    ...
    points.Set(i, point);
    fit_pts++; // ?è¨?ù∏?Ñ∞Î•? ?ã§?ùå ?öî?ÜåÎ°? ?ù¥?èô
  }
...
void parseEntityLine(Dwg_Object* object, Napi::Array jsonArr, Napi::Env env, bool isCheckLayer) {
  ...
  // 'lineObj' Í∞ùÏ≤¥?óê 'start'??? 'end' Î∞∞Ïó¥?ùÑ ?Ñ§?†ï
...
void parseEntityLwPolyline(Dwg_Object* object, Napi::Array jsonArr, Napi::Env env, bool isCheckLayer) {
  ...
  for (BITCODE_BL i = 0; i < lwPolyline->num_points; ++i) {
    ...
    points++; // ?è¨?ù∏?Ñ∞Î•? ?ã§?ùå ?öî?ÜåÎ°? ?ù¥?èô
  }
...
// DWG ?åå?ùº?ùÑ ?åå?ã±?ïò?äî C++ ?ï®?àò
Napi::Value ParseDWG(const Napi::CallbackInfo& info) {
  ...
  if (!info[1].IsArray()) {
    ...
    // Î∞∞Ïó¥ ?öî?ÜåÎ•? ?àú?öå?ïòÎ©? Î≥??ôò
    for (uint32_t i = 0; i < length; i++) {
        Napi::Value element = inputArray[i];

        if (element.IsString()) {
            Napi::String napiStr = element.As<Napi::String>();
            std::string cppStr = napiStr.Utf8Value();  // C++ Î¨∏Ïûê?ó¥Î°? Î≥??ôò
            ...
            // Î¨∏Ïûê?ó¥?ù¥ ?ïÑ?ãå Í≤ΩÏö∞ null ?è¨?ù∏?Ñ∞ Ï∂îÍ??
...
// Node.js??? ?ó∞Í≤∞Îêú ?Ñ§?ù¥?ã∞Î∏? ?ï†?ìú?ò® ?†ï?ùò
